...
def metadata_text():
    return "\n".join([
        "Metadata-Version: 2.4", "Name: waveframe-guard", "Version: 0.19.0",
...
def test_metadata_rejects_missing_or_incorrect_license(expression):
    ...
    with pytest.raises(AssertionError, match="License-Expression: Apache-2.0"):
        package_acceptance._validate_metadata(metadata, "0.19.0", "distribution")
...
def archive_files(kind):
    ...
    prefix = "waveframe_guard-0.19.0.dist-info/" if kind == "wheel" else ""
    ...
    doc_prefix = "waveframe_guard-0.19.0.data/data/share/doc/waveframe-guard/" if kind == "wheel" else ""
...
def test_archive_checks_require_both_packaged_notices(tmp_path, kind, missing):
    ...
    if kind == "wheel":
        ...
        with tarfile.open(path, "w:gz") as archive:
            for name, value in files.items():
                info = tarfile.TarInfo("waveframe_guard-0.19.0/" + name)
                info.size = len(value)
    ...
    if missing:
        ...
        assert inspect(path, "0.19.0") == len(files)
